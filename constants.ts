
export const OFFICE_LOCATION = {
  address: "F. Mendoza Commercial Complex, Units 102 & 103, Brgy. 28, Sto. Niño St., Tacloban City, Philippines, 6500",
  lat: 11.2420268, // Approximate latitude for Tacloban City center
  lng: 125.0034135, // Approximate longitude for Tacloban City center
  radius: 50 // in meters
};

export const DEPARTURE_TIME_LIMIT = "17:00"; // 5 PM

export const ALLOWED_WIFI = [
  { name: "GlobeAtHome_E863F_5", ip: "222.127.77.195" },
  { name: "PLDTHOMEFIBR5G11a58", ip: "124.105.78.208" },
  { name: "GlobeAtHome_E863F_2.4", ip: "222.127.77.195" },
  { name: "PLDTHOMEFIBR11a58", ip: "124.105.78.208" },
  { name: "dlink", ip: "222.127.77.195" }
];
